...
if(isset($_POST["versturen"])){
	if($_POST["Keuzeproduct"] == "Start"){
		echo '<div class="alert alert-danger" role="alert">Je moet een product meegeven. </div>';
	}
	else{
	$_SESSION["keyCategorie"] = $_POST["Keuzeproduct"];
	header("location:voegtoe.php");
	}
}
?>
<!DOCTYPE html>
...
<body>
  ...
  <div class="container">
    ...
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active">Categorieen</li>
    </ol>
	<form name="sentMessage" id="contactForm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
		<select name="Keuzeproduct">
		  ...
		  {
			  $sql = "SELECT productid, productnaam FROM tblproducten";
			  $res = mysqli_query($mysqli, $sql);
			  while($row = $res->fetch_assoc()){
				  $pnaam = $row["productnaam"];
				  $pid = $row["productid"];
				  echo '<option value="'.$pid.'">--'.$pnaam.'--</option>';
			  }
	  	  }
			   ?>
		</select>
